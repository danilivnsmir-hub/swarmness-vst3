...
```
вход → [SMOKE pre] → STING → HIVE → SWARM → [SMOKE post] → WINGS → VOLUME → выход
```
...
**OUTPUT** — **VOLUME**. Общего MIX нет: у каждой секции свой баланс (MIX в STING и SWARM, BLEND в SMOKE, DRONE/QUEEN в HIVE), поэтому фузз и хорус не разбавляются чистым сигналом.
...
**FACTORY** — 28 заводских пресетов в пяти категориях (подменю); каждый показывает одну возможность, описание всплывает при наведении на имя пресета:
