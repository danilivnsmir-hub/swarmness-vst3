...
**Рой разъярённых пчёл в виде плагина: октавы, диссонанс, гармонии с «ядом» регенерации, хорус, фузз, гейт, два эквалайзера и реверб в переставляемой цепи — для дэткора, металкора и хардкора.**
...
```
вход → INPUT → [ SMOKE ][ PITCH ][ SWARM ][ WINGS ][ COMB ][ CARVE ][ CRYPT ] → VOLUME → выход
                          └ PITCH = STING ∥ HIVE (голоса)

любые блоки можно поставить параллельно:
вход → [ SMOKE ][ PITCH ] ─┬─ A: [ SWARM ] ──┬─ MIX A/B → [ WINGS ] … → выход
                            └─ B: [ CRYPT ] ──┘
```
...
- **клик** по плитке открывает страницу блока: **FX** (STING/HIVE, SWARM, SMOKE, WINGS), **EQ** (COMB и CARVE) или **CRYPT**;
- **клик по индикатору** на плитке включает/выключает блок (у PITCH индикатор только показывает активность: HIVE включён или нажат футсвич октавы);
- **параллельно:** потяните плитку **вверх** — она уйдёт в параллельную ветку **A**, **вниз** — в ветку **B**, обратно в середину — снова последовательно. Цепь раздваивается на месте первого параллельного блока и сливается после веток. Пустая ветка = чистый сигнал (**DRY**): например, SMOKE в ветке A и пустая B — классический параллельный фузз с подмесом чистого. Мини-ручка в точке слияния — баланс **A/B** (в центре обе ветки по половине: одинаковые ветки дают ровно исходный уровень). Ветки выровнены по задержке, так что фузз в одной ветке не фазит с другой.

Порядок и ветки сохраняются в пресетах и в сессии; при перестановке звук на ~8 мс приглушается, щелчков нет. По умолчанию: SMOKE → PITCH → SWARM → WINGS → COMB → CARVE → CRYPT. Старые сессии и пресеты с SMOKE **POST** открываются с SMOKE после SWARM.
...
- **ROOM / PLATE / HALL / ABYSS** — алгоритмические пространства (сеть из 8 линий задержки с диффузией и модуляцией; ABYSS — огромный, тёмный, движущийся);
- **IR** — ваш импульсный отклик: **LOAD IR** или просто перетащите WAV/AIFF/FLAC на страницу (до 12 с, моно или стерео). Путь к файлу сохраняется в сессии и в пользовательских пресетах;
...
Общего MIX нет: у каждой секции свой баланс (MIX в STING, HIVE, SWARM и CRYPT, BLEND в SMOKE), поэтому фузз и хорус не разбавляются чистым сигналом.
...
**FACTORY** — 37 заводских пресетов в шести категориях (подменю); каждый показывает одну возможность, описание всплывает при наведении на имя пресета:
...
- **Chain, EQ & Crypt:** Crypt Doom, Smoke in the Crypt (реверб *в* фузз), Tight Before Smoke (CARVE перед фуззом), Comb Smile, Hive Cathedral, Parallel Smoke (фузз ∥ чистый), Split Swarm (хорус ∥ реверб), Chug Room
...
./build/SwarmnessTests_artefacts/Release/SwarmnessTests --screenshot ui.png "Hive Collapse" 1.5 0   # страница: 0 FX, 1 EQ, 2 CRYPT
```
...
Source/
  ...
  PluginEditor.*          главное окно (масштабируемое), страницы FX / EQ / CRYPT
  ...
  GUI/                    тема, LookAndFeel, элементы управления, ChainStrip (цепь), Pages (EQ, CRYPT)
...
Swarmness — закрытое ПО © Insect Audio, все права защищены (см. `LICENSE`). Сторонние компоненты перечислены в `THIRD_PARTY_NOTICES.md`; весь DSP написан с нуля, GPL-кода нет.
